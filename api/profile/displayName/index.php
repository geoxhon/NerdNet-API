...
if($_SERVER["REQUEST_METHOD"] == "GET"){
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_fetch($stmt)){
                    ...
                    $jsonResult->triggerResults["displayName"] = $bio;
                    die(json_encode($jsonResult));
                }else{
                     $jsonResult->success=false;
    ...
    if($data["displayName"]==null){
        $jsonResult->success=false;
        $jsonResult->reason="Validation error, displayName can't be null";
        http_response_code(400);
        die(json_encode($jsonResult));
    }elseif(strlen($data["displayName"])>50){
        $jsonResult->success=false;
        $jsonResult->reason="Validation error, displayName must be less than 50 characters";
        http_response_code(400);
        die(json_encode($jsonResult));
    }
    if(strlen($data["displayName"])<5){
        $jsonResult->success=false;
        $jsonResult->reason="Validation error, displayName cant be less than 5 characters";
        http_response_code(400);
        die(json_encode($jsonResult));
    }
